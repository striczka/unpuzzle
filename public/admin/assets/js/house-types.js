new Vue({
    ...
    methods: {
        ...
        getCities:function(href){
            var vue = this;
            $.get(!!href ? href : "/dashboard/house-types", {"_token": this.token}).done(function(cities){
        ...
        addCity: function(){
            var vue = this;
            $.post("/dashboard/house-types", {"_token": this.token, "name": this.city.name }).done(function(createdCity){
        ...
        updateCity: function(){
            var vue = this;
            $.post("/dashboard/house-types/" + this.city.id, {"_token": this.token,"_method": "PUT", "name": this.city.name}).done(function(updatedCity){
        ...
        deleteCity: function (city, event) {
            event.preventDefault();
            if(confirm("Вы действительно хотите удалить этот тип дома?")){
                var vue = this;
                $.post("/dashboard/house-types/" + city.id, {"_token": this.token, "_method": "delete"}).done(function(){

new Vue({
    ...
    methods: {
        ...
        getCities:function(href){
            var vue = this;
            $.get(!!href ? href : "/dashboard/house-class", {"_token": this.token}).done(function(cities){
        ...
        addCity: function(){
            var vue = this;
            $.post("/dashboard/house-class", {"_token": this.token, "name": this.city.name }).done(function(createdCity){
        ...
        updateCity: function(){
            var vue = this;
            $.post("/dashboard/house-class/" + this.city.id, {"_token": this.token,"_method": "PUT", "name": this.city.name}).done(function(updatedCity){
        ...
        deleteCity: function (city, event) {
            event.preventDefault();
            if(confirm("Вы действительно хотите удалить этот класс дома?")){
                var vue = this;
                $.post("/dashboard/house-class/" + city.id, {"_token": this.token, "_method": "delete"}).done(function(){
